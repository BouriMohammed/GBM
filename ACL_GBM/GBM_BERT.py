...
xtrain_ids = np.load('./data/imdb/x_train_bert_id.npy')
xtrain_masks = np.load('./data/imdb/x_train_bert_masks.npy')

xtest_ids = np.load('./data/imdb/x_test_bert_id.npy')
xtest_masks = np.load('./data/imdb/x_test_bert_masks.npy')

y_train = np.load('./data/imdb/y_train.npy')
y_test = np.load('./data/imdb/y_test.npy')

x_interval = np.load('./inputs/x_interval_bert.npy')

train_dataset = utils.IMDBBERTDataset(
    input_ids=xtrain_ids, attention_masks=xtrain_masks, targets=y_train)
valid_dataset = utils.IMDBBERTDataset(
    input_ids=xtest_ids, attention_masks=xtest_masks, targets=y_test)
...
class Engine:
    ...
    def train(self, data_loader, epoch, num_epochs, input_interval, hidden_size, ka, kb, kg):
        ...
        for batch in tqdm(data_loader, desc=f'Epoch {epoch + 1}/{num_epochs}', leave=False):
            ...
            targets = batch['targets'].unsqueeze(1).float().to(self.device)

            self.optimizer.zero_grad()
            outputs = self.model(input_ids, attention_mask, train_mode=True)
    ...
    def evaluate(self, data_loader, input_interval, hidden_size, ka, kb, kg):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, desc='Evaluating', leave=False):
                ...
                outputs = self.model(
                    input_ids, attention_mask, train_mode=False)
...
model = BiLSTM(bert_model, hidden_size, 1, 1)
...
# folder_path = f'./model/imdb/bert/{ka}_{kb}/'
